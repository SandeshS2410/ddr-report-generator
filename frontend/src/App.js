                ...
                <div className="inline-img-caption">
                  ...
                  <span className="img-badge-meta">
                    ...
                    {typeof img.match_confidence === 'number' &&
                      ` · match ${(img.match_confidence * 100).toFixed(0)}%`}
                  </span>
                </div>
              </div>
            );
      ...
      if (img) {
        htmlBody += `<div style="margin:12px 0;">
          ...
          <div style="font-size:11px;color:#666;margin-top:4px;">
            ...
            ${typeof img.match_confidence === 'number' ? ` · match ${(img.match_confidence*100).toFixed(0)}%` : ''}
          </div>
        </div>`;
      } else {
...
export default function App() {
  ...
  const generate = async () => {
    ...
    setResult(null);
    setRawReport('');
    setStatusMsg('Extracting areas and thermal readings…');
    ...
    try {
      setStatusMsg('Matching thermal scans to inspection areas (this can take a minute)…');
          ...
          <section className="panel result-panel">
            ...
            <div className="stats-bar">
              <div className="stat-item">
                <span className="stat-val">{result.stats.total_areas}</span>
                <span className="stat-label">Areas found</span>
              </div>
              <div className="stat-item">
                <span className="stat-val">{result.stats.areas_with_thermal_match}</span>
                <span className="stat-label">Thermal-matched</span>
              </div>
              <div className="stat-item">
                <span className="stat-val">{result.stats.areas_without_thermal_match}</span>
                <span className="stat-label">No thermal match</span>
              </div>
              <div className="stat-item">
                <span className="stat-val">{result.stats.total_inspection_images}</span>
                <span className="stat-label">Inspection photos</span>
              </div>
              <div className="stat-item">
                <span className="stat-val">{result.stats.total_thermal_readings}</span>
                <span className="stat-label">Thermal pages</span>
              </div>
            </div>

            {result.stats.areas_without_thermal_match > 0 && (
              <div className="status-msg info" style={{ marginBottom: 16 }}>
                {result.stats.areas_without_thermal_match} area(s) had no thermal scan confidently
                matched by visual comparison — these are marked "Not Available" in the report
                rather than guessed.
              </div>
            )}
                        ...
                        {group.map(img => (
                          ...
                          >
                            <img
                              ...
                              alt={img.id}
                            />
                            <div className="img-thumb-label">
                              {img.source} p.{img.page}
                              {typeof img.match_confidence === 'number' &&
                                ` · ${(img.match_confidence * 100).toFixed(0)}%`}
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        <div className="lightbox" onClick={() => setActiveImg(null)}>
          <div className="lightbox-inner" onClick={e => e.stopPropagation()}>
            ...
            <div className="lightbox-meta">
              ...
              {typeof activeImg.match_confidence === 'number' &&
                ` · match ${(activeImg.match_confidence * 100).toFixed(0)}%`}
            </div>
          </div>
        </div>
      )}
